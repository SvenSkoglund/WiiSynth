...
public class WRLImpl extends WiiRemoteAdapter {
	...
	public static void main(String args[]) {
		...
		try {
			...
			while (remote == null) {
				try {
					remote = WiiRemoteJ.connectToRemote("191D6F34D9");
					// remote = WiiRemoteJ.findRemote();
					System.out.println("in findRemote try");
				} catch (Exception e) {
					System.out.println("in findRemote Exception");
					remote = null;
					e.printStackTrace();
			...
			// remote = WiiRemoteJ.connectToRemote(" ");
	...
	public void accelerationInputReceived(WRAccelerationEvent evt) {
		...
		if (accelerometerSource) {
			...
			osc.frequency.set(baseFreq + freqShift);
			osc2.frequency.set(osc2baseFreq + freqShift * osc2interval);
			osc3.frequency.set(osc3baseFreq + freqShift * osc3interval);
		 ...
		 * evt.getXAcceleration()); System.out.println("Y: " + evt.getYAcceleration());
		...
		try {
			if (status == 0) {
				if (accelerometerStatus == 0)
					...
					remote.setMouse(
							new MotionAccelerometerMouse(80, 60, AccelerometerMouse.NUNCHUK_EXTENSION, 0.06, 0.08));
				else if (accelerometerStatus == 3)
					remote.setMouse(new TiltAccelerometerMouse(10, 10, AccelerometerMouse.NUNCHUK_EXTENSION, 0.1, 0.1));
			} else if (status == 1)
				...
				if (analogStickStatus == 0)
					...
					remote.setMouse(new AbsoluteAnalogStickMouse(1, 1, AnalogStickMouse.CLASSIC_CONTROLLER_LEFT));
				else if (analogStickStatus == 3)
					remote.setMouse(
							new RelativeAnalogStickMouse(10, 10, 0.05, 0.05, AnalogStickMouse.CLASSIC_CONTROLLER_LEFT));
				else if (analogStickStatus == 4)
					remote.setMouse(new AbsoluteAnalogStickMouse(1, 1, AnalogStickMouse.CLASSIC_CONTROLLER_RIGHT));
				else if (analogStickStatus == 5)
					remote.setMouse(new RelativeAnalogStickMouse(10, 10, 0.05, 0.05,
	...
	public void buttonInputReceived(WRButtonEvent evt) {
		...
		if (evt.wasPressed(WRButtonEvent.MINUS)) {
			...
			osc3baseFreq = baseFreq * osc3interval;
			System.out.println("Minus");
		}
			...
			if (evt.isPressed(WRButtonEvent.HOME)) {
				...
				if (evt.wasPressed(WRButtonEvent.A + WRButtonEvent.B)) {
					...
					remote.getButtonMaps()
							.add(new ButtonMouseMap(WRButtonEvent.B, java.awt.event.InputEvent.BUTTON1_MASK));
					remote.getButtonMaps()
							.add(new ButtonMouseMap(WRButtonEvent.A, java.awt.event.InputEvent.BUTTON3_MASK));
					...
					remote.getButtonMaps()
							.remove(new ButtonMouseMap(WRButtonEvent.B, java.awt.event.InputEvent.BUTTON1_MASK));
					remote.getButtonMaps()
							.remove(new ButtonMouseMap(WRButtonEvent.A, java.awt.event.InputEvent.BUTTON3_MASK));
							...
							try {
								...
								if (!dataF.exists()) {
									...
									for (int c = 0; c < data.length; c++) {
										// System.out.println("0x" + Integer.toHexString(data[c]) + " : 0x" +
										// Integer.toHexString(dataO[c]));
										if (data[c] != dataO[c])
								...
								/*
								 * byte[] data2 = remote.readData(new byte[]{0x04, (byte)0xA2, 0x00, 0x00},
								 * 65535); System.out.println("Read complete (data2)"); if (!data2F.exists()) {
								 * DataOutputStream data2OS = new DataOutputStream(new
								 * FileOutputStream(data2F)); data2OS.write(data2, 0, data2.length);
								 * data2OS.close(); } else { System.out.println("Comparing arrs (data2)"); for
								 * (int c = 0; c < data2.length; c++) { System.out.println("0x" +
								 * Integer.toHexString(data2[c]) + " : 0x" + Integer.toHexString(data2O[c])); if
								 * (data2[c] != data2O[c])System.out.println("Register: 0x" +
