...
/**
 * Display each oscillator's waveform using the AudioScope. This is a
 * re-implementation of the TJ_SeeOsc Applet from the old API.
 *
 * @author Phil Burk (C) 2010 Mobileer Inc
 */
public class SeeOscillators extends JApplet implements WiiRemoteListener {
	...
	private static UnitOutputPort oscFreq;
	...
	public SeeOscillators(WiiRemote remote) {
		this.remote = remote;
	}

	/* Can be run as either an application or as an applet. */
	public static void main(String args[]) {
		oscFreq = new UnitOutputPort();

		SeeOscillators applet = new SeeOscillators(remote);
		...
		try {
			...
			remote.addWiiRemoteListener(new SeeOscillators(remote));
			remote.setAccelerometerEnabled(true);
			remote.setSpeakerEnabled(true);
			remote.setIRSensorEnabled(true, WRIREvent.BASIC);
			remote.setLEDIlluminated(0, true);
		} catch (Exception e) {
			System.out.println("Error intitializing Wii Remote");
		}
	...
	public void start() {
		...
		synth.add(this.freqRamp = new LinearRamp());
		freqRamp.input.setup(50.0, 50, 20000.0);
	...
	private void addOscillator(final UnitOscillator osc, String label) {
		...
		checkBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				widthSlider.setEnabled(osc instanceof PulseOscillator || osc instanceof PulseOscillatorBL);
...
//			System.out.println(freqShift);
			oscFreq.setValueInternal(freqShift);;
			t++;
